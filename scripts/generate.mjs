...
const getApiConfig = async () => {
	const results = await new cfn.CloudFormationClient({
		region: 'us-east-1',
	}).send(new cfn.DescribeStacksCommand({}))

	console.log('ok...the results', results)
	const stack = results.Stacks.filter(
		(stack) => stack.StackName === 'TestStack'
	)[0]
	console.log('the stack stuff', stack)
	const apiKeyOutputs = stack.Outputs.filter(
		(output) => output.OutputKey === 'GraphQLAPIKeyOutput'
	)
	const graphqlurlOutputs = stack.Outputs.filter(
		(output) => output.OutputKey === 'GraphQLAPIEndpointOutput'
	)
	const apiIdOutputs = stack.Outputs.filter(
		(output) => output.OutputKey === 'GraphQLAPIIdOutput'
	)
	return {
		...(apiKeyOutputs.length === 1
			? { apiKey: apiKeyOutputs[0].OutputValue }
			: {}),
		...(graphqlurlOutputs.length === 1
			? { graphqlUrl: graphqlurlOutputs[0].OutputValue }
			: {}),
		...(apiIdOutputs.length === 1
			? { apiId: apiIdOutputs[0].OutputValue }
			: {}),
	}
}

const downloadInstrospectionSchema = async (apiId) => {
	const results = await new appsync.AppSyncClient().send(
		new appsync.GetIntrospectionSchemaCommand({
			apiId,
...
const invokeCodegen = (apiId) =>
	new Promise((resolve) => {
		const codegen = pty.spawn('amplify', [
			'codegen',
			'--apiId',
			['codegen', '--apiId', apiId],
		])
